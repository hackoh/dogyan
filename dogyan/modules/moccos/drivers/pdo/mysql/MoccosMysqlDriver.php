 ...
 * @version 2.0
 */
class MoccosMysqlDriver extends MoccosPDODriver {
	...
	public function insert(Moccos $instance) {
		...
		self::$_caches[$this->_className] = array();
	...
	public function select(MoccosFinder $finder) {
		//vd('selectstart:'.memory_get_usage());
		$cacheKey = md5(serialize($finder));
		if (isset(self::$_caches[$this->_className][$cacheKey])) {
			return self::$_caches[$this->_className][$cacheKey];
		}
		...
		if ($finder->filter->isEmpty() === false)
			$subqueries[] = sprintf('where %s', self::renderFilter($finder->filter));
		...
		//$bt = ob_get_clean();
		MoccosLog::push(MOCCOS_LOG_INFO, sprintf('query = %s.
		', $query));
		$stmt = $this->_pdo->query($query);
		if ($stmt !== false) {
		...
		//vd('selectend:'.memory_get_usage());
		//self::$_caches[$this->_className][$cacheKey] = $result;
		//return self::$_caches[$this->_className][$cacheKey];
		return $result;
	}
		...
		switch ($filter->type) {
			case '&&':
				...
				foreach ($filter->args as $_filter_) {
					$renders[] = $this->renderFilter($_filter_);
				}
			...
			case '%.%':
				return sprintf("%s like '%%%s%%'", $filter->args['property'], $filter->args['value']);
			default:
				if ($filter->args['value'] === true || $filter->args['value'] === false)
					$filter->args['value'] = (int)$filter->args['value'];
		...
		foreach ($properties as $property) {
			if (! $property->isRelationProperty()) {
				$columns[] = sprintf('%s %s %s %s %s',
					...
					$property->not_null ? 'not null' : null,
